'''Notch — Phase 2, step 1: the intelligent contract.

One contract, four responsibilities:

  IDENTITY THAT COSTS SOMETHING (S27). Sellers post a slashable bond. Their
  wallet key is the key receipts must come from, so "who is this seller" is
  answered by economics, not self-declaration.

  TERMS BOUND BEFORE PAYMENT. A quote — criteria, window, amount — is hashed
  and registered before any buyer signs. A dispute can only ever be judged
  against terms that predate the money.

  EXPOSURE RESERVED AT ACCEPTANCE (S23). Recording a payment reserves the
  slashable portion against the seller's bond immediately. A seller cannot
  accept more concurrent payments than their bond could answer for.

  JUDGMENT ONLY WHERE JUDGMENT IS NEEDED. Non-delivery is deterministic (no
  receipt, no release — then refund). The panel runs only for the one genuine
  question: whether what the receipt vouches for met the published criteria.

Two GenVM traps this file deliberately avoids, both paid for on siblings:
runtime DynArray instantiation (EchoGen — per-wallet lists are JSON in a
TreeMap instead) and @staticmethod on contract methods (GroundTruth CI,
genvm-lint E022 — helpers are module-level functions).

Run from the project root:  python scripts/phase2/10_contract.py
'''
...
path = ROOT / "contracts" / "notch.py"
path.parent.mkdir(parents=True, exist_ok=True)
path.write_text(CONTRACT, encoding="utf-8")
print(f"  wrote  {path.relative_to(ROOT)}  ({len(CONTRACT.splitlines())} lines)")
print("\nPhase 2 / step 1 complete — the contract exists. Now lint it, test it, break it.")
